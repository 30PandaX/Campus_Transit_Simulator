...
RtestConfigManager::RtestConfigManager() : routes(std::vector<Route *>()) {
}
...
bool RtestConfigManager::ReadConfig(const std::string filename) {
    ...
    while (std::getline(configFile, line)) {
        ...
        if (chunk == "ROUTE_GENERAL") {
            ...
            if (stops.size() > 0) {
                ...
                routes.push_back(
                        new Route(
                            ...
                            new RtestPassengerGenerator(currProbabilities,
                                                         stops)));
            ...
            // This means moving 1 speed in a time click moves 1 mile. That's a
            // bit far, so I multiply * 2 so that a speed of 1 moves 1/2 mile
            latitude *= 69 * 2;
            longitude *= 55 * 2;
